...
int Read_file(const char * infilename, map<string, unsigned int> (&table)[7]){
    ...
    while(getline(infile, buffer)){
                          ...
                          /*
                          int str_iter = 0;
                          while ((bio_level < 10) && (str_iter < str_length)){
                                if ((buffer[str_iter]== ' ')||(buffer[str_iter] == '\t')){
                                      if (bio_level >3){ //column > 2
                                                    if (temp == "Unclassified;") break;
                                                    if (temp.find("unclassified")!=string::npos) break;
                                                    if (temp.find("other")!=string::npos) break; 
                                                    if (table[bio_level-4].count(temp) == 0)
                                                       table[bio_level-4][temp] = 1;
                                                    else table[bio_level-4][temp] ++;
                                                    }
                                      temp = "";
                                      bio_level ++;
...
int Show_table(map<string, unsigned int> table[6], int line_count, ostream & out){
    string label[7] = {"界", "门", "纲", "目", "科", "属", "种"};
    //for (int i = 0; i < 6; i++)
    {
      int i =1;  
        float temp = 0;
